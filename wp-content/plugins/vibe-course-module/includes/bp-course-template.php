...
function bp_course_has_items( $args = '' ) {
	...
	function bp_course_get_avatar( $args = array() ) {
		...
		if(has_post_thumbnail($id)){
			$thumb='<a href="'.get_permalink($id).'" title="'.the_title_attribute('echo=0').'" itemprop="image">'.get_the_post_thumbnail($id,$size).'</a>';
		}else{
			$default_course_avatar = vibe_get_option('default_course_avatar');
			if(isset($default_course_avatar) && $default_course_avatar){
				$thumb='<a href="'.get_permalink($id).'" title="'.the_title_attribute('echo=0').'" itemprop="image"><img src="'.$default_course_avatar.'" /></a>';
			}
		}
...
if(!function_exists('bp_course_get_course_credits')){
	function bp_course_get_course_credits($args=NULL) {
			...
			if(isset($product_id) && $product_id !='' && function_exists('get_product')){ //WooCommerce installed
				...
				if(is_object($product)){
        			...
        			if(isset($check) && $check)
        				$link.='?redirect';
					$credits[$link] = '<strong itemprop="price">'.$product->get_price_html().'</strong>';
				}
			}
			...
			$levels=pmpro_getAllLevels();
				foreach($levels as $level){
					if($level->id == $membership_id){
						$link=get_option('pmpro_levels_page_id');$link=get_permalink($link);
		...
		return apply_filters('wplms_course_credits',$credits_html,$id);
	}
}
...
if(!function_exists('bp_course_get_students_undertaking')){
	function bp_course_get_students_undertaking($course_id=NULL, $page=0){ // Modified function, counts total number of students
		...
		$cquery=$wpdb->prepare("SELECT DISTINCT meta_key FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_value >= %d AND meta_key REGEXP '^-?[0-9]+$' ORDER BY meta_value ASC LIMIT %d, %d",$course_id,0,$page_num,$loop_number);
	...
	if(isset($course_curriculum) && count($course_curriculum)){
		...
		if(!$flag){
			...
			if(vibe_validate($auto_eval)){
    			...
    			do_action('badgeos_wplms_evaluate_course',$id,$marks,$user_id);
			    ...
			    if(vibe_validate($passing_cert) && isset($passing_per) && $passing_per && $marks > $passing_per){
			        ...
			        bp_course_record_activity(array(
			          ...
			          'content' => __('Student ','vibe').bp_core_get_userlink($user_id).__(' got a certificate in the course ','vibe').get_the_title($id),
