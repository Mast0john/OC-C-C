...
function wplms_assignment_answer_posted() {
  ...
  $count = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->comments} WHERE comment_post_ID = %d and comment_approved = %d and user_id = %d",$post_id,1,$user_id));
...
function wplms_assignments_stats($list){
  $list['stats_student_assignment_marks']= __('Assignment scores','vibe');
  return $list;
}
