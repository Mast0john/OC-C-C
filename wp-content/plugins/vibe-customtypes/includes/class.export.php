...
class wplms_export{
        ...
        if(is_numeric($specific)){
       	...
       	if($results->have_posts()){
       		while($results->have_posts()){
       			...
       			if($content){
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_type',__('Type','vibe_customtypes'),$post->post_type);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_title',__('Title','vibe_customtypes'),$post->post_title);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_date',__('Date','vibe_customtypes'),$post->post_date);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_content',__('Content','vibe_customtypes'),$post->post_content);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_author',__('Author ID','vibe_customtypes'),$post->post_author);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_excerpt',__('Excerpt','vibe_customtypes'),$post->post_excerpt);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_status',__('Status','vibe_customtypes'),$post->post_status);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'post_name',__('Slug','vibe_customtypes'),$post->post_name);
       				$this->csv[]=array($wpdb->prefix.'posts',$post->ID,'comment_count',__('Review/Note/Answer Count','vibe_customtypes'),$post->comment_count);
       			}
	...
	public function get_module_data($id){
		...
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_type',__('Type','vibe_customtypes'),$result->post_type);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_title',__('Title','vibe_customtypes'),$result->post_title);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_date',__('Date','vibe_customtypes'),$result->post_date);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_content',__('Content','vibe_customtypes'),$result->post_content);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_author',__('Author ID','vibe_customtypes'),$result->post_author);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_excerpt',__('Excerpt','vibe_customtypes'),$result->post_excerpt);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_status',__('Status','vibe_customtypes'),$result->post_status);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'post_name',__('Slug','vibe_customtypes'),$result->post_name);
		$this->csv[]=array($wpdb->prefix.'posts',$result->ID,'comment_count',__('Review/Note/Answer Count','vibe_customtypes'),$result->comment_count);
	...
	function get_taxonomy($post_id,$post_type){
		...
		if(isset($terms) && is_array($terms)){
			...
			if(is_array($terms_array) && count($terms_array))
				$this->csv[]=array($wpdb->prefix.'terms',$post_id,$taxonomy,__('Terms','vibe_customtypes'),implode(',',$terms_array));
		}
	...
	function get_comments($id){
		...
		if ( $comments ) {
			foreach ( $comments as $comment ) {
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'comment_post_ID',__('Connected Post','vibe_customtypes'),$comment->comment_post_ID);
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'comment_type',__('Type','vibe_customtypes'),$comment->comment_type);
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'comment_content',__('Content','vibe_customtypes'),$comment->comment_content);
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'comment_date',__('Date','vibe_customtypes'),$comment->comment_date);
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'comment_parent',__('Parent ID','vibe_customtypes'),$comment->comment_parent);
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'user_id',__('Author ID','vibe_customtypes'),$comment->user_id);
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'comment_author',__('Author Name','vibe_customtypes'),$comment->comment_author);
				$this->csv[]=array($wpdb->prefix.'comments',$comment->comment_ID,'comment_author_email',__('Author Email','vibe_customtypes'),$comment->comment_author_email);
