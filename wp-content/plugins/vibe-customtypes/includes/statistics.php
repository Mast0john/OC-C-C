				...
				<div class="postbox course_info">
					...
					<div class="inside">
						...
						if(isset($_GET['paged']) && $_GET['paged']){
							echo '<a href="?page=lms-stats&tab=course&subtab='.$_GET['subtab'].'&paged='.($_GET['paged']-1).'" class="button">&lsaquo; '.__('Prev').'</a>';
						}
						if($total == $num){
							if(isset($_GET['paged']) && $_GET['paged']){
								$paged =$_GET['paged'];
							...
							echo '<a href="?page=lms-stats&tab=course&subtab='.$_GET['subtab'].'&paged='.($paged+1).'" class="button">'.__('Next').' &rsaquo;</a>';
						}
						...
						<?php
							...
							foreach($terms_array as $k=>$term){
								if(isset($term['avg']) && is_Array($term['avg'])){ 
									$d=array_sum($term['avg']) / count($term['avg']);
...
global $wpdb;
	...
	<div id="poststuff" class="vibe-reports-wrap">
		<div class="vibe-reports-sidebar">
			...
			<div class="postbox">
				<h3><span><?php _e( 'Total Active Students', 'vibe' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $active_students > 0 ) echo $active_students; else _e( 'n/a', 'vibe' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total Course Students ', 'vibe' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($course_students>0) echo $course_students; else _e( 'n/a', 'vibe' ); ?></p>
				</div>
			</div>
		</div>
		<div class="vibe-reports-main">
				<div class="postbox course_info">
					...
					<div class="inside">
						...
						<?php
						if(isset($_GET['paged']) && $_GET['paged']){
							echo '<a href="?page=lms-stats&tab=students&subtab=overview&paged='.($_GET['paged']-1).'" class="button">&lsaquo; '.__('Prev').'</a>';
						}
						if($total == $num){
							if(isset($_GET['paged']) && $_GET['paged']){
								$paged =$_GET['paged'];
							...
							echo '<a href="?page=lms-stats&tab=students&subtab=overview&paged='.($paged+1).'" class="button">'.__('Next').' &rsaquo;</a>';
						}
